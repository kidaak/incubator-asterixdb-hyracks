...
public class LSMBTree extends AbstractLSMIndex implements ITreeIndex {
    ...
    public void markAsValid(ILSMComponent lsmComponent) throws HyracksDataException {
        ...
        int bloomFilterFileId = component.getBloomFilter().getFileId();
        IBufferCache bufferCache = component.getBTree().getBufferCache();
        int startPage = 0;
        int maxPage = component.getBloomFilter().getNumPages();
        forceFlushDirtyPages(bufferCache, bloomFilterFileId, startPage, maxPage);
        if (collectStatistics) {
            maxPage = component.getStatistics().getNumPages();
            int statisticsFileId = component.getStatistics().getFileId();
            forceFlushDirtyPages(bufferCache, statisticsFileId, startPage, maxPage);
        }
